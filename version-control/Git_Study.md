# Git 설치 후 기본 설정
## VScode 에디터 설정
Git 충돌 시 VScode에서 시각적으로 비교하고 해결할 수 있게 함.
...
## 설치 중 bash only X window prompt 옵션 선택
Git Bash 설치 과정 중에 선택할 수 있는 옵션으로 편의를 위해 선택한다.  
이 옵션을 선택하면 Git Bash 환경에서 Git 명령어와 Unix 명령어를 사용할 수 있어서 편의성이 높아진다.


# GitLens
VScode 확장으로 설치한다.  
변경사항, 브랜치, 리모트, 히스토리 등 Git 관련 작업을 VScode 환경에서 시각적으로 확인할 수 있다.
