...
여기에서, 중간자 공격이란?
클라이언트와 서버 간 통신 중 제 3자가 중간에서 데이터를 가로채거나 조작하는 공격을 의미한다.
...
여기에서, 안전한 통신 채널이란?
클라이언트와 서버가 공유 대칭키를 가져서 데이터를 암호화, 복호화하며 통신하는 환경을 말한다.
...
> 쿼리스트링: URL 주소 뒤에 key-value로 파라미터를 포함하여 전송하는 부분
...
> Q. PUT vs PATCH
> PUT과 PATCH는 모두 서버 리소스를 업데이트하는 메소드라는 공통점이 있다.
